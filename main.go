...
func main() {
	...
	authTransport := util.WarpAuthTransport(*registryURL, *uname, *passwd, *insecure)

	// Create registry object
	r, err := client.NewRegistry(*registryURL, authTransport)
	if err != nil {
	...
	for _, entry := range entries[:numFilled] {
		...
		repo, err := client.NewRepository(repoName, *registryURL, authTransport)
		if err != nil {
		...
		for _, tag := range tagsData {
			...
			if err != nil {
				tagLogger.Error("Could not Unmarshal payload!")
				tagFetchDataErrors = true
				break
			}
			...
			if err != nil {
				tagLogger.Error("Could not Unmarshal blobInfo!")
				tagFetchDataErrors = true
				break
			}
